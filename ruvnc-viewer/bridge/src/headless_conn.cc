...
#include "ruvnc-viewer/src/bridge.rs.h"
...
void VncConnection::disconnect()
{
    if (!connected_)
        return;
    connected_ = false;
    pixelBuffer_ = nullptr;  // owned by CConnection, deleted by close()
    close();
}
...
bool VncConnection::processMessages()
{
    try {
        ...
        // EAGAIN/EWOULDBLOCK means the send buffer is temporarily full;
        // treat as a transient condition, not a fatal error.
        if (e.err == EAGAIN || e.err == EWOULDBLOCK) {
            try { getOutStream()->cork(false); } catch (...) {}
            return false;
        }
...
void VncConnection::sendKeyPress(uint32_t keyCode, uint32_t keySym)
{
    try {
        ...
        if (e.err == EAGAIN || e.err == EWOULDBLOCK) return;
        throw;
    }
}
...
void VncConnection::getUserPasswd(bool secure, std::string* user,
                                  std::string* password)
{
    auto result = on_get_credentials(secure, user != nullptr);
    if (!result.ok) {
