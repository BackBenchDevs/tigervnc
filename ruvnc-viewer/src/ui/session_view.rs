...
use crate::renderer::{CursorData, FrameBuffer, Renderer, ZoomMode};
...
impl SessionView {
    ...
    pub fn show(&mut self, ui: &mut Ui) {
        ...
        // Zoom toolbar at the bottom
        egui::TopBottomPanel::bottom("zoom_bar").show_inside(ui, |ui| {
            ui.horizontal(|ui| {
        ...
        let available = ui.available_size();
        let result = self.renderer.render_to_egui(ui, available);

        if let Some((response, img_rect)) = result {
            self.handle_input(ui, &response, &img_rect, fb_w, fb_h);
        }
    }
    ...
    fn handle_input(
        ...
        if let Some(pos) = response.hover_pos() {
            let scale_x = fb_w as f32 / img_rect.width();
            let scale_y = fb_h as f32 / img_rect.height();
            let rel_x = pos.x - img_rect.left();
            let rel_y = pos.y - img_rect.top();
