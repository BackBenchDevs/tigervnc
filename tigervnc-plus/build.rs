...
fn generate_config_h(out_dir: &PathBuf, cmake_build_dir: &PathBuf) {
...
fn build_vnc_core(common_dir: &PathBuf) {
    ...
    build
        ...
        .define("PACKAGE_VERSION", "\"1.16.80\"")
        .define("BUILD_TIMESTAMP", "\"tigervnc-plus\"")
...
fn build_bridge(common_dir: &PathBuf, bridge_dir: &PathBuf) {
    let out_dir = PathBuf::from(std::env::var("OUT_DIR").unwrap());
    let sources = vec![
