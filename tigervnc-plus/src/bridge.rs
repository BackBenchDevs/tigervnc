...
pub mod ffi {
    ...
    extern "Rust" {
        ...
        fn on_get_credentials(secure: bool) -> CredentialResult;
...
fn on_get_credentials(secure: bool) -> ffi::CredentialResult {
    log::info!("Credential request (secure={})", secure);
    ...
    // First check if we already have credentials (pre-provided or from keyring)
    {
        ...
        if !host.is_empty() {
            log::debug!("Checking keyring for {}:{} user='{}'", host, port, username);
            ...
            log::debug!("No stored password found in keyring for {}:{}", host, port);
        }
    }
